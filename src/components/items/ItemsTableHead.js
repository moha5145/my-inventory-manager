...
const TableHeader = () => {
  return tHeaderText.map((text, index) => {
    return <>
      <th
        key={index}
        className='first:w-4 w-20 md:w-52 h-12 text-left text-xs sm:text-md mb-[0.017rem]'
      >
        {text}
      </th>
    </> 
  ...
  return (
    <thead className="w-full text-white sm:[&>*:not(:first-child)]:hidden">
        { itemsState.items.map((item) => {
          return ( 
            <tr key={item.id} className="px-1  bg-blue-400 flex flex-col flex-no wrap justify-center sm:table-row rounded-l-lg sm:rounded-none mb-5 sm:mb-0">
              <TableHeader />
            </tr>
          )})
        }
