...
const EnterVsOutValue = () => {
  const [period, setPeriod] = useState('day');

  const data = {
    day: {
      labels: [...Array(8)].map((_, i) => `${i * 3}:00`),
      enter: [...Array(8)].map(() => Math.floor(Math.random() * 1000)),
      out: [...Array(8)].map(() => Math.floor(Math.random() * 1000)),
    },
    week: {
    ...
    <div className="h-96 w-full md:w-1/2 xl:w-1/2 p-4">
      <div className="bg-white shadow-md p-4 h-full">
        <div className="flex justify-between mb-2">
          <h2 className="text-lg font-bold">Valeur Sortie vs Entrée</h2>
...
export default EnterVsOutValue;